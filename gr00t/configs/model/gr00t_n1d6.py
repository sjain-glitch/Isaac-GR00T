...
class Gr00tN1d6Config(PretrainedConfig):
    ...
    # rtc_delay_max = 0 => OFF (standard scalar-time flow matching, unchanged). When ON, each
    # sample draws a delay d ~ Uniform{rtc_delay_min .. rtc_delay_max} (INCLUSIVE) and freezes
    # the first d action tokens at t=1 (clean, ground-truth), training ONLY the suffix — so the
    # model learns to continue from a frozen executed prefix (deploy: hard-freeze the executed
    # prefix + plain TRT forward, no VJP). Set the range to the DEPLOY latency band ONLY:
    # d = ceil(latency * control_hz); 120-200 ms @ 30 Hz -> d in {4,5,6} -> min=4, max=6.
    rtc_delay_min: int = 0
    rtc_delay_max: int = 0
